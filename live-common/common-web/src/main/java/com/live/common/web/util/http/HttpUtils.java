package com.live.common.web.util.http;
...
//		{"code":0,"data":{"ip":"114.218.96.52","country":"中国","area":"","region":"江苏","city":"苏州","county":"XX","isp":"电信","country_id":"CN","area_id":"","region_id":"320000","city_id":"320500","county_id":"xx","isp_id":"100017"}}
    ...
    public synchronized static Map<String, String> ipParse(String ip) throws IOException {
        Map<String, String> map = new HashMap<String, String>();
        try {
            if (StringUtils.isNotBlank(ip) && !"127.0.0.1".equals(ip) && !"0:0:0:0:0:0:0:1".equals(ip)) {
     ...
     * @throws Exception
     */
    public synchronized static Map<String, String> getAddressWhois(String ip) {
        Map<String, String> map = new HashMap<String, String>();
        CloseableHttpClient httpClient = HttpClients.createDefault();
        ...
        try {
            ...
            WhoisBean wb = JSONUtil.toBean(content, WhoisBean.class);
            if (wb != null) {
                map.put("province", StringUtils.isBlank(wb.getPro()) ? wb.getAddr() : wb.getPro());
                map.put("city", wb.getCity());
            }
        } catch (Exception e) {
            log.error("getAddressWhois occur error. ip:{}", ip, e);
        } finally {
            try {
                if (httpResponse != null) {
                    httpResponse.close();
