...
import com.indo.user.mapper.MemGoldChangeMapper;
...
public class MemGoldChangeServiceImpl extends ServiceImpl<MemGoldChangeMapper, MemGoldChange> implements IMemGoldChangeService {
    ...
    public boolean updateMemGoldChange(MemGoldChangeDTO change) throws RuntimeException {
        ...
        long begin;
        RReadWriteLock lock = redissonClient.getReadWriteLock(UserConstants.UPDATE_USER_BALANCE_ + change.getUserId());
        try {
            // 写锁
            ...
            if (bool) {
                ...
                MemBaseinfo memBaseinfo = memBaseInfoService.getMemBaseInfoById(Long.valueOf(userId));
                if (memBaseinfo == null) {
                ...
                int row = memBaseInfoService.updateMemberAmount(changeAmount, changeCanAmount, memBaseinfo.getId());
                if (row != 1) {
