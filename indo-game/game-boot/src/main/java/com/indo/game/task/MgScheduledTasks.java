...
public class MgScheduledTasks {
    ...
    public void reportCurrentTime() throws MalformedObjectNameException {
        if (taskRun) {
            String key = "pullMgBetOrder" + DateUtils.format(new Date(), DateUtils.longFormat);
            ...
            try {
                ...
                if (bool) {
                    if (RedisBaseUtil.get(key) == null) {
                        logger.info("=============== pullMgBetOrder order pull is begin " + dateFormat.format(new Date())
                                + " ===============");
                        mgService.pullMgBetOrder();
                        RedisBaseUtil.set(key, "1", 60L);
                        logger.info("=============== pullMgBetOrder order pull is end " + dateFormat.format(new Date())
                                + " =============");
                    }
                }
            } catch (Exception e) {
                logger.error(" pullMgBetOrder is error {} ", e);
            }
        }
    }
}
