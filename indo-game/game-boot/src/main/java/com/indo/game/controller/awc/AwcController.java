...
public class AwcController {
    ...
    @AllowAccess
    public Result initGame(@LoginUser LoginInfo loginUser, String isMobileLogin, String gameCode, String platform, HttpServletRequest request) throws InterruptedException {
        logger.info("AE真人、SV388斗鸡log {} initGame 进入游戏。。。loginUser:{}", loginUser.getId(), loginUser);
        String params = "";
        ...
        try {
            if (res) {
                ...
                if (resultInfo == null) {
                    logger.info("AE真人、SV388斗鸡log {} initGame result is null. params:{},ip:{}", loginUser.getId(), params, ip);
                ...
                logger.info("AE真人、SV388斗鸡log {} initGame resultInfo:{}, params:{}", loginUser.getId(), JSONObject.toJSONString(resultInfo), params);
                return resultInfo;
            } else {
                logger.info("AE真人、SV388斗鸡log {} initGame lock  repeat request. error");
                String aeInitGame3 = MessageUtils.get("networktimeout");
                return Result.failed(aeInitGame3);
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("AE真人、SV388斗鸡log {} initGame occur error:{}. params:{}", loginUser.getId(), e.getMessage(), params);
            return Result.failed(MessageUtils.get("networktimeout"));
        } finally {
            lock.unlock();
        }
    ...
    public Result logout(@LoginUser LoginInfo loginUser,String userIds, HttpServletRequest request) throws InterruptedException {
        logger.info("AE真人、SV388斗鸡log {} logout 进入游戏。。。loginUser:{}", loginUser.getId(), loginUser);
        String params = "";
        if (loginUser == null) {
            return Result.failed(MessageUtils.get("ParameterError"));
        }
        try {
            ...
            e.printStackTrace();
            logger.error("AE真人、SV388斗鸡log {} logout occur error:{}. params:{}", loginUser.getId(), e.getMessage(), params);
            return Result.failed(MessageUtils.get("networktimeout"));
        }
    }
}
