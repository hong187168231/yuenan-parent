...
@EnableScheduling
@Component
public class AwcScheduledTasks {
    ...
    public void reportCurrentTime() throws MalformedObjectNameException {
        if (taskRun) {
            String key = "awcTask" + DateUtils.format(new Date(), DateUtils.longFormat);
            ...
            try {
                ...
                if (bool) {
                    if (RedisBaseUtil.get(key) == null) {
                        ...
                        logger.info("=============== awcPullOrder pullOrder pull is begin {} ===============", dateFormat.format(new Date()));
                        awcAeSexybcrtService.awcPullOrder("");
                        logger.info("=============== awcPullOrder pullOrder pull is end {} =============", dateFormat.format(new Date()));
                    }
                }
            } catch (Exception e) {
